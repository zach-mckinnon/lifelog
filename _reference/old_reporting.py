...
from lifelog.config.config_manager import load_config, get_tracker_definition
from lifelog.commands.utils.shared_options import period_option
...
from lifelog.commands.utils.tracker_utils import sum_entries, count_entries
...
def render_trend_chart(tracker_name, entries):
    ...
    fig.plot(list(range(len(avg_values))), avg_values,
             ...
             ylabel=Text(tracker_name, style="italic"),
...
def report_help():
    ...
    """
    console.print("[bold blue]📊 Available Report Types:[/bold blue]")
...
@app.command()
def time(period: str = period_option):
    """
   Show [bold yellow]time tracking totals[/bold yellow] by category.
    ...
    if period == "day":
        ...
        console.print("[bold red]Invalid period.[/bold red] …")
...
@app.command("habits")
def report_habits(period: str = period_option):
    """
    Show [bold green]number of completions[/bold green] per tracker.
    """
    now = datetime.now()
    if period == "hour":
    ...
    counts = {}
    # get all trackers that have count goals
    config = cf.load_config()
    for name, t in config.get("trackers", {}).items():
        for g in t.get("goals", []):
            if g["type"] == "count":
                counts[name] = count_entries(name, since=period)
...
def daily():
    ...
    for name, t in config.get("trackers", {}).items():
        for g in t.get("goals", []):
            if g["type"] == "count":
                habit_counts[name] = count_entries(name, since="today")
...
@app.command("trend")
def report_trend(metric: str, period: str = period_option):
    """
    ...
    filtered = [e for e in entries if e.get("tracker") == metric and datetime.fromisoformat(e["timestamp"]) > since]
...
def report_heatmap(kind: str):
    ...
    if kind == "time":
        ...
        # Treat kind as tracker name
        t = cf.get_tracker_definition(kind)
        date_map = defaultdict(float)
        for g in t.get("goals", []):
            if g["type"] == "sum":
                # sum per day
                daily_sum = sum_entries(kind, since="month")  # or adjust period
                date_map["today"] = daily_sum
            else:
                # count per day
                daily_count = count_entries(kind, since="month")
                date_map["today"] = daily_count
