...
from lifelog.utils.db.models import Tracker, TrackerEntry, goal_from_row
...
def add_tracker(tracker):
    conn = get_connection()
    cur = conn.cursor()
    # If using a dataclass, convert to dict first
    if hasattr(tracker, '__dict__'):
        tracker = tracker.__dict__
    # Or if using asdict from dataclasses:
    # from dataclasses import asdict
    # tracker = asdict(tracker)

    cur.execute("""
        INSERT INTO trackers (title, type, category, created)
        VALUES (?, ?, ?, ?, ?)
    """, (
        tracker.get('title'),
        tracker.get('type'),
